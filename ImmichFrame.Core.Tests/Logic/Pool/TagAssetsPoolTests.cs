...
public class TagAssetsPoolTests
{
    ...
    private static AssetResponseDto Asset(string id) => new() { Id = TestIds.From(id), Type = AssetTypeEnum.IMAGE };
    ...
    public async Task LoadAssets_PaginatesAndCombinesMultipleTags()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(result.Any(a => a.Id == TestIds.From("t1_p1_0")));
            Assert.That(result.Any(a => a.Id == TestIds.From("t1_p2_29")));
            Assert.That(result.Any(a => a.Id == TestIds.From("t2_19")));
        });
    }
    ...
    public async Task LoadAssets_EmptyTagResultsAreIgnored()
    {
        ...
        var expectedIds = assets.Select(a => a.Id).ToHashSet();
    ...
    public async Task LoadAssets_AttachesTagsDirectlyWithoutExtraApiCalls()
    {
        ...
        _api.Verify(a => a.GetAssetInfoAsync(It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<string>(), default), Times.Never);
    }
    ...
    public async Task LoadAssets_DeduplicatesAssetsWithMultipleTags()
    {
        ...
        Assert.That(result.Count(a => a.Id == TestIds.From("shared")), Is.EqualTo(1));
        ...
        var sharedAsset = result.First(a => a.Id == TestIds.From("shared"));
        Assert.Multiple(() =>
        {
